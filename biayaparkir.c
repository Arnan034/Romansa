...
Kendaraan daftarKendaraan[100]={0}; // maksimal jumlah kendaraan yang diparkir adalah 100 untuk mobil dan motor
int sizeKendaraan=0;
char NPlat[15]={'\000'};

void input_ID_password();
/*Prosedur untuk petugas memasukan ID nya
I.S : login belum ditampilakan
F.S : menginputkan username dan password petugas untuk mengecek identitasnya telah terdaftar.
*/

void readKendaraanMasuk();
...
void hapusKendaraanByPosisi(int posisiKdrn);
/*Prosedur menghapus kendaraan di daftar list kendaraan
I.S. : Kendaraan belum dihapus
F.S. : Kendaraan sudah dihapus, elemen kendaraan selanjutnya bergeser ke kiri
*/
...
int main(){
	...
	while(opsi != 99){		
        ...
        continue;
		switch(opsi){
			case 1: LihatKendaraan();
					break;
			case 2: readKendaraanMasuk();
					break;
			case 3: do{
						printf("\nNomor Plat Motor: ");
						scanf(" %[^\n]", &NPlat);
						ketemu = cariKendaraanByPlat(sizeKendaraan);
						lagi = 0;
						if(ketemu==0){
					...
					if(ketemu==1){
						posisiKendaraanKeluar = getPosisiKendaraanByPlat();
						kendaraan = getKendaraanByPlat();
						readJam(&jKluar);
						setJamKeluarKendaraan(&kendaraan,jKluar);
						hargaParkir = calculationParkir(kendaraan);
						hapusKendaraanByPosisi(posisiKendaraanKeluar);
...
void input_ID_password(){
	...
	ID = 12345678;
	do {
		printf("Masukan ID (12345678): ");
		scanf("%i", &username); // input username dengan angka "12345678"
		printf("Masukan password(12345678): ");
...
void readKendaraanMasuk(){
	...
	strcpy(daftarKendaraan[sizeKendaraan].nomorPlat,NPlat);
	daftarKendaraan[sizeKendaraan].tipeKendaraan = type;
	daftarKendaraan[sizeKendaraan].jamMasuk = JMasuk;
	sizeKendaraan++;
}
...
int cariKendaraanByPlat(int sizeKendaraan){
	...
	while((index<=sizeKendaraan) && (result == 0)){
		if(NPlat!=daftarKendaraan[sizeKendaraan].nomorPlat){
			index++;
		}else{
			result = 1;
		}
...
Kendaraan getKendaraanByPlat(){
	...
	while((index <= sizeKendaraan) && (result == 1)){
		if (daftarKendaraan[index].nomorPlat == NPlat){ //disini masalahnya
														//padahal harunya udah sama dua-duanya
			result = index;
		}
		index++;
	}
